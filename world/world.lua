...
function worldToMap(w_x, w_y)
    -- Calculate position on the map(add 1 to account for block size and 0.1 for rounding error)
    return math.floor(w_x + map.center.x + 1.8), math.floor(w_y + map.center.y + 1.8)
end